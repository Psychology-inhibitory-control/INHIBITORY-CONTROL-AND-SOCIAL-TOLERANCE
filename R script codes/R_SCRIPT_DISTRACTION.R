...
DATA_mean<-subset(DATA_ALL_STROOP, session==1)
  
  
DATA_m=aggregate(DATA_ALL_STROOP_S1$perf, by=list(DATA_ALL_STROOP_S1$subj_id, DATA_ALL_STROOP_S1$tol), mean, na.rm=TRUE)

colnames(DATA_m) = c('subj_id','tolerance', 'perf' )
...
DATAREACTION<-read.csv("C:/Users/Marine/Desktop/DATA_STRAS/REACTION2.csv", header=TRUE)
...
########FOR TOLERANCE
library(pscl)
model_tol<- zeroinfl(Nb_indiv ~ type_pict + tolerance ,
                 data =DATAREACTION,
                 dist = "poisson")
summary(model_tol)
...
ggplot( mapping = aes(x=tolerance, y=Nb_indiv, fill=type_pict)) +
  scale_fill_viridis(discrete = T, option="plasma")+
  geom_bar(stat="identity", position = position_stack(reverse=TRUE)) + 
  guides(fill = guide_legend(reverse=TRUE))+


  labs(x = "\nDegree of Tolerance", y = "Number of Individual having a behavioural response \n") + labs(fill="Type of picture")+
  theme_classic(base_size = 22)+
  scale_x_discrete(labels=c("Low", "Medium", "High")) +
  theme_classic(base_size = 22)
